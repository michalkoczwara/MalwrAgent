# -*- coding: utf-8 -*-
"""the chain helper is the class of reference for any action regarding a client chain. it is used to create a chain,
validate its module's order and parameters."""
...
class Chain(object):
    """the chain class consists of all methods needed to create a client's chain and
    is used as a subclass by the client"""
    ...
    def __validate_config(self, module_idx, module_args):
        ...
        def __do_validate():
            ...
            if module_args is None:
                ...
                if function in ATTR_ARGS and (
                                module_idx < 1 or (
                                            module_idx >= 1 and config and not config.get('hybrid', False))):
                    # only if module is not first item in chain and is a hybrid

                    self.logger.log_debug('Checking ' + function + ' with args ' + str(user_provided_arguments))
                    ...
                    if required_args and user_provided_arguments is None:
                        _result['reason'] = 'Arguments missing: ' \
    ...
    def add_item(self, chain_id, module, module_args, mode='auto'):
        """add an chain item to the chain"""
        ...
        if result['result'] is True:
            self.logger.log_info(convert_to_text() + ' done')
            self.chain[chain_id].append((module, module_args))
        elif result['result'] is False:
            self.logger.log_critical(convert_to_text() + ' failed')
    ...
    def get_formatted_chain(self, out_format='json'):
        """get the chain formatted as desired"""
        if out_format == 'json':
    ...
    def load_chain_from_file(self, filename):
        """load the chain configuration from a file"""
        with open(filename) as infile:
            for item in json.load(infile):
                ...
                if not self.add_item(chain_id, module, {'settings': settings}):
                    return False
    ...
    def save_chain_to_file(self, filename):
        """save the chain configuration to a file"""
        chain = []
